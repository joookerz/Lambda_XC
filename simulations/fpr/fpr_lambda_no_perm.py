"""Null (no-structure) false positive rate for λ_XC without permutations.

Simulates a single panmictic population (no structure), computes λ_emp from
cross-chromosome pairs, and estimates the empirical false positive rate using
the decision rule λ_emp >= λ_threshold (default 1.0, matching the power tests).
...
def run(cfg: Config) -> None:
    ...
    for N in cfg.N_grid:
        for rep in range(cfg.replicates):
            ...
            rows.append({
                ...
                "replicate": rep,
                "lambda_emp": lam,
                "detect_vs_one": int(lam >= cfg.lambda_threshold),
            })

    out_dir = cfg.output_dir
    out_dir.mkdir(parents=True, exist_ok=True)
    details = pd.DataFrame(rows)
    details.to_csv(out_dir / "fpr_no_perm_details.csv", index=False)
    ...
    summary.to_csv(out_dir / "fpr_no_perm_summary.csv", index=False)
    print(f"Wrote {out_dir/'fpr_no_perm_details.csv'} and {out_dir/'fpr_no_perm_summary.csv'}")


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Null FPR for λ_XC without permutations")
    ...
    p.add_argument("--lambda-threshold", type=float, default=1.0)
    p.add_argument("--seed", type=int, default=2025)
    p.add_argument("--output-dir", type=Path, default=Path("simulations/fpr/output_null_no_perm"))
